...
def main() -> int:
    ...
    require(text, '"/api/manual-hive/wechat/folo-open"', "前端缺少 Folo 订阅打开接口调用")
    ...
    if "RSS源" in text:
        raise AssertionError("公众号卡片不能再暴露原始 RSS 源按钮")
    require(text, "在 Folo 订阅并打开", "公众号卡片需要提供 Folo 订阅并打开按钮")
    require(text, "data-wechat-folo-open", "前端缺少 Folo 订阅打开按钮绑定")
    require(text, "openManualHiveWechatInFolo", "前端缺少 Folo 订阅打开动作")
    require(text, "copyText(data.feed_url)", "Folo 打开前必须复制公开 RSS feed URL")
