...
- 附件协议已新增，附件存储在 `uploads/`，消息通过 `attachments` 引用；当前正在优化为 GPT 只传 `attachment_ids`，由 AgentHub 服务端展开附件元数据。
...
1. 用轻量 `attachment_ids` 协议复测附件调度，避免 GPT 工具调用携带完整附件对象。
2. 单独研究阶段 1F：Windows Codex App 私有窗口自动投递。
